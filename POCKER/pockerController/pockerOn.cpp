...
uint32_t *pockerOn::enter(OpenCard *cards) {
...
pockerOn::PlayerData::~PlayerData() {
    delete[] hand;
    delete p;
}
...
std::pair<OpenCard *, uint32_t *> pockerOn::strongestCombination(OpenCard *source) {
...
std::vector<uint32_t> pockerOn::refresh() {
...
//do players turn
void pockerOn::doBets(uint32_t pls) {
    ...
    do {
        if (players[curr].cash != 0) {
            std::cout << "<<" << (*players[curr].p).getName() << ">> turn\n";
...
std::string pockerOn::rankToString(uint32_t in) {
    switch (in) {
        case 14:
            return "A";
        case 13:
...
void pockerOn::show(Player *p, OpenCard *cards, uint32_t *value) {
...
std::vector<uint32_t> pockerOn::show() {
...
void pockerOn::openNext() {
    CloseCard chosen = deck.back();
    deck.erase(--deck.end());
    table.push_back(OpenCard(chosen.suit, chosen.rank));
}

Board pockerOn::getTable(uint32_t player, std::vector<uint32_t> live) {
...
size_t pockerOn::inGameWithCash() {
    size_t cnt = 0;
    for (uint32_t i = 0; i < live.size(); i++) {
